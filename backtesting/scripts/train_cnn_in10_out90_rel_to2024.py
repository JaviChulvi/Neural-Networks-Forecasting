"""Entrena el CNN Deep Conv1D (input=10, output=90) con target relativo al universo.

Igual que train_cnn_in10_out90_to2024.py pero usando relative=True en
train_cnn_window: el target y es el exceso de retorno de cada activo respecto
a la media del universo en la ventana de salida, en lugar de la media absoluta.

El modelo queda guardado en:
    data/cnn/saved_models/cnn_input10_output90_rel_model.keras

Uso desde terminal:
    python backtesting/scripts/train_cnn_in10_out90_rel_to2024.py
    CNN_EPOCHS=50 FORCE_RETRAIN=1 python backtesting/scripts/train_cnn_in10_out90_rel_to2024.py

Uso desde otro script o notebook:
    from backtesting.scripts.train_cnn_in10_out90_rel_to2024 import run
...
def run(
    ...
    """Entrena el CNN (input=10, output=90, relative=True) sobre returns_to2024.parquet.
