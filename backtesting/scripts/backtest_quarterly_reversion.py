...
bench_reversion_zscore_90d   : Long top-4 activos más alejados por debajo de su media móvil
                               de log-precio en 90 días.
bench_reversion_zscore_input : Long top-4 activos más alejados por debajo de su media móvil
                               usando INPUT_WINDOW días.
bench_reversion_longshort    : Long bottom-4 z-score 90d / short top-4 z-score 90d;
...
---------------------------------------
    data/backtest/portfolio_values_reversion_{model}[_rel]_in{INPUT}_out{OUTPUT}.csv
    data/backtest/metrics_summary_reversion_{model}[_rel]_in{INPUT}_out{OUTPUT}.csv
    data/backtest/cumulative_return_reversion_{model}[_rel]_in{INPUT}_out{OUTPUT}.png
"""
...
def setup(
    ...
    """
    global model, scaler, returns_full, log_prices, assets, n_assets
    ...
    log_prices = returns_full.cumsum()
...
def mean_reversion_zscore(date: pd.Timestamp, lookback_days: int) -> pd.Series:
    """Z-score de log-precio usando solo datos anteriores a la fecha de rebalanceo.

    z < 0 indica que el activo cotiza por debajo de su media móvil reciente.
    Las estrategias de reversión compran los z-scores más bajos.
    """
    iloc = returns_full.index.get_loc(date)
    start = max(0, iloc - lookback_days)
    window = log_prices.iloc[start:iloc]
    if len(window) < 2:
        raise ValueError(
            f"No hay suficientes datos previos a {date.date()} para lookback={lookback_days}."
        )
...
def strat_reversion_zscore_90d(date: pd.Timestamp, **_) -> dict:
    """Long top-4 activos más alejados por debajo de su media móvil de 90 días."""
    z = mean_reversion_zscore(date, TRAILING_VOL_DAYS)
    bottom = np.argsort(z.values)[:N_TOP]
    return {assets[i]: 1.0 / N_TOP for i in bottom}


def strat_reversion_zscore_input(date: pd.Timestamp, **_) -> dict:
    """Long top-4 z-score bajo usando el mismo lookback que el modelo."""
    z = mean_reversion_zscore(date, INPUT_WINDOW)
    bottom = np.argsort(z.values)[:N_TOP]
    return {assets[i]: 1.0 / N_TOP for i in bottom}


def strat_reversion_longshort(date: pd.Timestamp, **_) -> dict:
    """Long bottom-4 z-score 90d / short top-4 z-score 90d."""
    z = mean_reversion_zscore(date, TRAILING_VOL_DAYS)
...
STRATEGIES = {
    ...
    "bench_reversion_zscore_90d":   strat_reversion_zscore_90d,
    "bench_reversion_zscore_input": strat_reversion_zscore_input,
    "bench_reversion_longshort":    strat_reversion_longshort,
...
def save_outputs(ret_df: pd.DataFrame, holdings: dict) -> None:
    ...
    tag = f"reversion_{MODEL_TYPE}{rel_part}_in{INPUT_WINDOW}_out{OUTPUT_WINDOW}"
    ...
    STYLE = {
        ...
        "bench_reversion_zscore_90d":   dict(color="purple",    lw=1.5, ls="-.", label="Reversión z-score 90d"),
        "bench_reversion_zscore_input": dict(color="darkgreen", lw=1.5, ls=":",  label=f"Reversión z-score {INPUT_WINDOW}d"),
        "bench_reversion_longshort":    dict(color="saddlebrown", lw=1.5, ls="-", label="Reversión Long-Short"),
    ...
    ax.set_title(
        f"Backtesting trimestral 2025 vs reversión a la media — {model_label} "
