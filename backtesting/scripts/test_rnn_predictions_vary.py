"""Verifica que las predicciones del modelo RNN varían entre instantes
...
El script admite tres parámetros opcionales:

    --model-type   lstm | gru            (default: lstm)
    --input-window  entero               (default: 10)
    --output-window entero               (default: 90)

Ejecución con pytest:
    pytest backtesting/scripts/test_rnn_predictions_vary.py -v
    pytest backtesting/scripts/test_rnn_predictions_vary.py --model-type gru -v
    pytest backtesting/scripts/test_rnn_predictions_vary.py --model-type gru --input-window 10 --output-window 90 -v

Ejecución directa:
    python backtesting/scripts/test_rnn_predictions_vary.py
    python backtesting/scripts/test_rnn_predictions_vary.py --model-type gru
    python backtesting/scripts/test_rnn_predictions_vary.py --model-type gru --input-window 10 --output-window 90
"""
...
# ── parámetros del modelo (leídos de pytest CLI o de argparse) ─────────────
@pytest.fixture(scope="module")
def params(pytestconfig):
    """Resuelve los parámetros del modelo desde las opciones de pytest."""
    model_type    = pytestconfig.getoption("--model-type")
    input_window  = pytestconfig.getoption("--input-window")
    output_window = pytestconfig.getoption("--output-window")
    model_path = (
        PROJECT_ROOT / "data" / "rnn" / "saved_models"
        / f"rnn_{model_type}_input{input_window}_output{output_window}_model.keras"
    )
...
@pytest.fixture(scope="module")
def inference_data(params):
    """Devuelve (X_sample, preds, indices) para N_SAMPLES instantes espaciados."""
    returns = load_returns(str(DATA_DIR), RETURNS_FILE)
    X, _ = create_time_series_data(returns, params.input_window, params.output_window)
    # X: (n_total, input_window, n_assets) — sin escalado (igual que en el notebook)

    n_total = len(X)
    min_step = params.input_window  # garantiza que ningún par comparte días de entrada
...
def test_model_file_exists(params):
    """El fichero .keras existe antes de intentar cargarlo."""
    assert params.model_path.exists(), f"Modelo no encontrado: {params.model_path}"
...
def test_inputs_differ_globally(inference_data):
    """Los N_SAMPLES inputs crudos no son todos iguales (std global > 0)."""
    X_sample, _, _ = inference_data
    std = X_sample.std()
    assert std > 1e-6, (
        f"std global de inputs = {std:.2e}. Los {N_SAMPLES} inputs son idénticos — "
        "bug en la selección de índices."
    )


def test_inputs_per_asset_differ(inference_data):
    """Para cada activo, los N_SAMPLES inputs varían a lo largo del tiempo."""
    X_sample, _, _ = inference_data
...
def test_predictions_vary_over_time(inference_data):
    """Para cada activo, la predicción cambia a lo largo de los N_SAMPLES instantes.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Verifica que las predicciones del modelo RNN varían con el input."
    )
    parser.add_argument(
        "--model-type", default="lstm", choices=["lstm", "gru"],
        help="Tipo de modelo RNN: lstm o gru  (default: lstm)",
    )
    parser.add_argument(
        "--input-window", type=int, default=10,
        help="Tamaño de la ventana de entrada en días  (default: 10)",
    )
    parser.add_argument(
        "--output-window", type=int, default=90,
    ...
    sys.exit(pytest.main([
        ...
        f"--input-window={args.input_window}",
        f"--output-window={args.output_window}",
        *extra_pytest_args,
    ]))
