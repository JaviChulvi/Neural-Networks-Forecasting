"""Verifica que las predicciones del MLP (in=10, out=90) varían entre instantes
...
Ejecución:
    ...
    python backtesting/scripts/test_mlp_predictions_vary.py
"""
...
MODEL_PATH = (
    PROJECT_ROOT
    / "data" / "mlp" / "saved_models"
    / "mlp_4x100_gelu_dropout_l2_input10_output90_model.keras"
)
...
def inference_data():
    ...
    X_flat   = X_sample.reshape(N_SAMPLES, -1)         # (N_SAMPLES, INPUT_WINDOW * n_assets)

    model = keras.models.load_model(MODEL_PATH)
    preds = model.predict(X_flat, verbose=0)           # (N_SAMPLES, n_assets)
    return X_sample, X_flat, preds, indices
...
def test_inputs_per_asset_differ(inference_data):
    ...
    _, X_flat, _, _ = inference_data
    # X_flat: (N_SAMPLES, INPUT_WINDOW * n_assets) → std por feature
    per_feature_std = X_flat.std(axis=0)
    n_zero = (per_feature_std <= 1e-6).sum()
    assert n_zero == 0, (
...
def test_predictions_vary_over_time(inference_data):
    ...
    """
    X_sample, _, preds, _ = inference_data
...
def test_first_vs_last_differ(inference_data):
    ...
    _, _, preds, indices = inference_data
