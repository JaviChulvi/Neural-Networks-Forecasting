"""Verifica que las predicciones del CNN (in=10, out=90) varían entre instantes
...
Ejecución:
    pytest backtesting/scripts/test_cnn_predictions_vary.py -v
    python backtesting/scripts/test_cnn_predictions_vary.py
"""
...
MODEL_PATH = PROJECT_ROOT / "data" / "cnn" / "saved_models" / "cnn_input10_output90_model.keras"
...
def inference_data():
    ...
    # X: (n_total, INPUT_WINDOW, n_assets)

    # Scaler ajustado sobre el 90 % inicial (misma partición train/test que en
    # cnn_utils.train_cnn_window con test_size=0.1 y shuffle=False)
    n_total = len(X)
    n_train = int(n_total * 0.9)
    n_tr, window, n_assets = X[:n_train].shape

    scaler = StandardScaler()
    scaler.fit(X[:n_train].reshape(n_tr, -1))
    X_scaled = scaler.transform(X.reshape(n_total, -1)).reshape(n_total, window, n_assets)
    ...
    X_sample = X_scaled[indices]  # (N_SAMPLES, INPUT_WINDOW, n_assets)
...
def test_inputs_differ_globally(inference_data):
    """Los 100 inputs escalados no son todos iguales (std global > 0)."""
    X_sample, _, _ = inference_data
    std = X_sample.std()
    assert std > 1e-6, (
        f"std global de inputs = {std:.2e}. Los 100 inputs son idénticos — "
        "bug en el escalado o en la selección de índices."
    )
...
def test_inputs_per_asset_differ(inference_data):
    ...
    X_sample, _, _ = inference_data
    # X_sample: (N_SAMPLES, INPUT_WINDOW, n_assets) → aplanar tiempo e input_window
    # Queremos std por activo colapsando las dos primeras dimensiones
    ...
    assert n_zero == 0, (
        ...
        "Los inputs escalados no varían — bug en el StandardScaler."
    )
