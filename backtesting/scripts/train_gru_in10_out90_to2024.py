"""Entrena la GRU (input=10, output=90) sobre datos hasta 2024.

Equivalente a model/rnn/gru/hp_search_input10_output90.ipynb pero usando
...
El modelo queda guardado en:
    data/rnn/saved_models/rnn_gru_input10_output90_model.keras
...
Uso desde terminal:
    python backtesting/scripts/train_gru_in10_out90_to2024.py
    GRU_EPOCHS=300 python backtesting/scripts/train_gru_in10_out90_to2024.py

Uso desde otro script o notebook:
    from backtesting.scripts.train_gru_in10_out90_to2024 import run
    row = run(epochs=300)
"""
...
def run(
    epochs: int = int(os.getenv("GRU_EPOCHS", "200")),
    batch_size: int = int(os.getenv("GRU_BATCH_SIZE", "128")),
    bar_type: str | None = None,
    preprocessing_dir: Path | None = None,
    ffd: bool = False,
) -> dict:
    """Entrena la GRU (input=10, output=90) sobre returns_to2024.parquet.
    ...
    for i in range(GRU_LAYERS):
        ...
        model.add(GRU(UNITS, return_sequences=return_seq, dropout=DROPOUT))
    ...
    model_path = model_dir / f"rnn_gru_input{INPUT_W}_output{OUTPUT_W}{bar_suffix}_model.keras"
